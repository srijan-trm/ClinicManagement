    ...
    body.innerHTML = data.map(p => `
        <tr class="border-b border-slate-100 hover:bg-slate-50/80 transition-colors">
            ...
            <td class="py-5 px-8 font-bold text-slate-800 text-lg">
                ${p.name} <span class="text-sm font-medium text-slate-400 ml-2">${p.gender || ''}</span>
            </td>
            <td class="py-5 px-8">
            ...
            <td class="py-5 px-8">
                <div class="flex gap-2">
                    <button onclick="openHealthProfile(${p.patient_id})" class="px-4 py-2 bg-indigo-100 text-indigo-700 rounded-lg font-semibold hover:bg-indigo-200 transition">
                        View Profile
                    </button>
                    <button onclick="deleteRecord('patients', ${p.patient_id})" class="px-4 py-2 bg-rose-100 text-rose-700 rounded-lg font-semibold hover:bg-rose-200 transition">
                        Delete
                    </button>
                </div>
            </td>
